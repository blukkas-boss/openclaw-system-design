...
- **Purpose:** Catalog free LLMs across providers; reliability-weighted model rotation; multi-key auth rotation; whole-pool fallback chain.
- **DB:** free-llm/free_llm.db — tables: models, model_runs, key_runs
- **Providers:** OpenRouter (26), Cerebras (2), NVIDIA (120), Groq (16), Mistral (69), Apertis (553) = 786 free models
...
- **Whole-pool fallback chain:** `agent --run` walks ALL eligible free models — each exhausts its keys, falls THROUGH to next model. `--pool` default 0 (all eligible), `--max-models` cap available.
- **Auth profile caching:** `_AUTH_PROFILE_CACHE` per-process — long chains ~1s
- **Daily scan hardened:** 3× retry with backoff, `health` gate, failures logged to `logs/free-llm-failures.log` without wiping catalog
...
## voice_rag_agents/ — Voice RAG Pipeline
- **Purpose:** End-to-end voice-note RAG system with ingestion graph, query graph, embeddings, vector store, reranking, citation validation, and groundedness evaluation.
- **Status:** Waves 0–4 complete (git `619bfd6`). Wave 5 (testing workforce, golden eval datasets, CI, release candidate) pending.
- **Interfaces (frozen, Wave 1):** STTProvider, EmbeddingProvider, LLMProvider, VectorStore, Retriever, Reranker, PromptBuilder, CitationValidator, EvaluationScorer.
- **Gates:** 164 passed, 1 skipped (Milvus/Docker). ruff clean.
- **Stack:** LangGraph (ingestion + query graphs), FastAPI service, CLI, Docker Compose (API/Milvus/Ollama/OpenWebUI).
- **Default mode:** `VOICE_RAG_PROFILE=mock`. Real adapters wired via `profile=local`.
- **RAG quality thresholds (Wave 5):** Recall@5 ≥ 0.90, MRR ≥ 0.70, citation coverage ≥ 0.95.
- **Project root:** `/home/bobby/.openclaw/workspace/voice_rag_agents/`

## subagent-watchdog/ — Sub-Agent Failure Monitor
- **Purpose:** Zero-LLM watchdog that polls sub-agent runs and alerts on failures.
- **Components:** `watchdog.py` + `restart_policy.json`; systemd user timer (3-min poll).
- **Watch target:** `~/.openclaw/subagents/runs.json`
- **Alert method:** File-based `pending_alerts.jsonl` (cron CLI hits scope-approval from scripts, files are the reliable path).
- **Restart policy:** max 2 retries, 15-min cooldown, NO restart on rate_limit. Allowlist: vrag waves, book-writer.

## whitepaper-bot/ — Book Writer Pipeline
- **Purpose:** Standalone Telegram bot that captures notes via webhook, classifies them, and triggers autonomous book-writer agent.
- **Bot token:** 8938973504 (SEPARATE from main OpenClaw bot).
- **Webhook server:** FastAPI on port 8101.
- **Tunnel:** localhost.run SSH reverse tunnel → `whitepaper-tunnel.service` (auto-restart + auto webhook re-registration).
- **Ingest:** Webhook → noise filter (`_is_noise()` skips commands, ≤2 chars, bare hashtags) → DB → classify → `.build_request` marker.
- **Trigger:** OpenClaw cron (15 min) → `needs_book_writer.py` (zero-LLM gate) → `sessions_send` to book-writer agent.
- **Mode B (emergent):** `chapters.yaml` starts empty, chapters grow from content via `emerge.py` + `coder-local`.

## local-scheduler/agent_health.py — Crashed-Agent Detector
- **Purpose:** Zero-LLM health check run by daily heartbeat. Detects crashed/stuck agents and failing cron jobs.
- **Sources:** `openclaw tasks list --status lost|timed_out|failed`, `openclaw cron list` (consecutiveErrors>0), `openclaw tasks audit` (stale TaskFlows).
- **Flags:** `--recent-days N` (default 1.5), `--restart` (re-triggers failing CRONs). Deliberately does NOT auto-restart TaskFlows/subagents.
...
- **Review cron:** Daily isolated agentTurn, model=owl-alpha (free, 1M ctx), fallbacks=[nemotron-3-super-120b:free, qwen3-coder:free]. Fixed 2026-06-16 (was using nemotron-3-ultra-550b:free which always timed out).
